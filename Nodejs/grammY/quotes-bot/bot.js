...
bot.command("start", async (ctx) => {
  await ctx.reply(
    `${(await getRandomQuote()).text}\n\nHi. I can send you random quotes from ${(await getRandomQuote()).count} quotes collected from <a href="https://type.fit/api/quotes">type.fit quotes API</a>. Use /quote or send me any text, I will reply with a quote.`,
    randomButton
  );
});
...
bot.on(["msg:text", "callback_query"], async (ctx) => {
  ...
  await ctx.reply(`${(await getRandomQuote()).text}`, randomButton);
});
